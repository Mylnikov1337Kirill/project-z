# Agent Trail repo context inventory

Заполняется и обновляется перед AI-assisted разработкой в Agent Trail. Цель -- дать агенту достаточно контекста, не превращая handoff в бесконечный свиток.
...
Pair-matching slot UX PMX-02 note, 2026-06-04: follow-up docs live in `docs/product/pair-matching-slot-ux-plan.md` and `docs/product/pair-matching-slot-ux-subtasks.md`. The current shared `pair-matching` implementation treats filled targets as one-to-one assignments: the PMX-01 guard prevents occupied targets from being overwritten and rejects duplicate target readiness, and PMX-02 adds a right-side filled-slot `x` clear affordance while disabling target buttons occupied by another item. PMX-03 hover/focus polish, PMX-04 dedicated regression coverage and PMX-05 final browser QA/closeout remain next.

Answer display shuffle ADS-01 note, 2026-06-04: `docs/product/answer-display-shuffle-subtasks-2026-06-04.md` is complete through ADS-01. `src/features/mission/lib/answerDisplayShuffle.ts` exposes a generic `{ id: string }` seeded display shuffle plus launch-stable cache and localStorage repeat guard that stores only public ids; `answerDisplayShuffle.test.ts` covers the helper. The helper is not yet wired into mission UI. Next step is ADS-02: shuffle only `scenario-decision.options`, `chip-picker.chips` and the `chip-ordering` source bank in `MissionInteractionBoards.tsx`, preserving submit/scoring by id.
...
commands:
  ...
  docker_api_build: docker build -t agent-trail-api:local .
  docker_api_run_smoke: docker run --rm -d --name agent-trail-api-smoke -p 3000:3000 agent-trail-api:local
...
- [x] Phase 43 mentor personality pass for Russian-first Kilian gameplay copy.
